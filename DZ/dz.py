...
capture = cv2.VideoCapture('VID_20190508_000314.mp4')
...
prev_gray = cv2.CreateImage((width,height), 8, 1) #Will hold the frame at t-1
#prev_gray = image = np.zeros((height, width, 8), np.uint8)
#red = (255, 0, 0)
#rgb_color = red
#color = tuple(reversed(rgb_color))
#image[:] = color
gray = cv2.CreateImage((width,height), 8, 1) # Will hold the current frame
