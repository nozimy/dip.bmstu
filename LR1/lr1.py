...
class LR1():
    def __init__(self):
        ...
        self.trainAppleFolder = os.path.join(dir_path,  'Training/Apple Red 1/')
        self.trainPapayaFolder = os.path.join(dir_path, 'Training/Pear/')
        self.testAppleFolder = os.path.join(dir_path,   'Testing/Apple Red 1/')
        self.testPapayaFolder = os.path.join(dir_path,  'Testing/Pear/')
...
class model():
    ...
    def predictOne(self, image):
        ...
        for key, value in self.getTrainedClasses().items():
            ...
            dw = abs(w - features[1])
            dp = abs(p - features[2])
            if dh < self.eps["h"] and dw < self.eps["w"] and dp < self.eps["p"]: #and dp < self.eps["p"]:
                return key
        return "empty"
